...
class CheckpointManager:
    ...
    def print_experiments(self, experiments: List[Dict], show_details: bool = True, show_checkpoint_steps: bool = False):
        ...
        if show_details:
            print(f"{'Model':<20} {'Experiment':<30} {'Size':<10} {'Modified':<20} {'Checkpoints':<15} {'Wandb ID':<15}")
            print("-" * 120)
            ...
            for exp in experiments:
                ...
                print(f"{exp['model_type']:<20} {exp['name']:<30} {self.format_size(exp['size']):<10} "
    ...
    def manage_checkpoint_steps(self, experiment: Dict, keep_latest: int = None, 
                               delete_steps: List[int] = None, skip_steps: List[int] = None, 
    ...
    def manage_all_checkpoint_steps(self, experiments: List[Dict], keep_latest: int = None,
                                   delete_steps: List[int] = None, skip_steps: List[int] = None, 
                                   dry_run: bool = True) -> int:
    ...
    def interactive_delete(self, experiments: List[Dict], dry_run: bool = True):
        ...
        for i, exp in enumerate(experiments):
            ...
            print(f"  Modified: {exp['modified_time'].strftime('%Y-%m-%d %H:%M') if exp['modified_time'] else 'Unknown'}")
        ...
        while True:
            ...
            if choice.lower() == 'q':
                ...
                try:
                    ...
                    self.manage_checkpoint_steps(experiment, keep_latest=keep_count, skip_steps=skip_steps, dry_run=dry_run)
                    break
                except ValueError:
                ...
                try:
                    ...
                    self.manage_checkpoint_steps(experiment, delete_steps=step_nums, skip_steps=skip_steps, dry_run=dry_run)
                    break
                except ValueError:
...
def main():
    ...
    parser.add_argument("--list-only", action="store_true", 
                       help="Only list experiments, don't delete anything")
    parser.add_argument("--summary", action="store_true", 
                       help="Show summary only (no detailed table)")
    parser.add_argument("--show-steps", action="store_true",
                       help="Show detailed checkpoint steps information")
    
    # Checkpoint step management options
    parser.add_argument("--keep-latest-steps", type=int,
                       help="Keep only the latest N checkpoint steps in each experiment")
    parser.add_argument("--delete-steps", type=str,
    ...
    parser.add_argument("--manage-steps-only", action="store_true",
                       help="Only manage checkpoint steps, don't delete entire experiments")
    ...
    try:
        ...
        if args.keep_latest_steps or delete_steps or args.manage_steps_only:
